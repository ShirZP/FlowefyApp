...
def predict():
    ...
    try:
        ...
        if prediction == 'Unknown':
            return jsonify({"flower": prediction, "info": "Oops! I couldn't identify this flower.\nTry again with another image:)"})
        
        prediction_info = get_flower_info(prediction)

        return jsonify({"flower": prediction, "info": prediction_info})
    except Exception as e:
